...
def main():
	...
	for i in range(0, len(images), loop_inc):
		...
		if total_img_pixels > 30:  # Exclude images that contain little to no white pixels
			crop(images[i], parameters)  # Crop the images
			shape = shape_outliner(images[i])
			total_shape_pixels = np.count_nonzero(shape)
			if total_shape_pixels != 0:  # Because some all black images are "sneaking through", do a second check! Significantly slows down code
	...
	wb.save(save_as)  # Saves the excel file to code location
...
def excel_handler():
	...
	while not valid_file:
		...
		if old_excel == "Y" or old_excel == "y":  # User has selected a pre-existing workbook
			excel_file = input("Please specify the path to your excel file e.g., C-Users-You-Desktop-Folder-Test Img: ") + ".xlsx"
			try:
...
def data_writer(ws, porosity):
	ws.cell(row=1, column=1).value = "Trial name:"
	ws.cell(row=1, column=2).value = "Porosity:"
	g = 1  # We loop to find empty cells before outputting the results
	while ws.cell(row=g, column=1).value is not None and ws.cell(row=g, column=2).value is not None:
		g += 1
	ws.cell(row=g, column=1).value = "Trial %d" % (g - 1)
	ws.cell(row=g, column=2).value = porosity


# Randomly saves an image based on a tunable probability "odds" within functions
def random_saver():
	odds = 2  # Think of this as probability to save = 1/odds
